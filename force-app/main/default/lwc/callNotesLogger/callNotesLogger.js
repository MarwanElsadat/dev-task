...
export default class CallNotesLogger extends LightningElement {
    ...
    @wire(getRecentCallNotes, { opportunityId: '$recordId', limitSize: RECENT_LIMIT })
    wiredRecent(result) {
    ...
    handleContactChange(event) { this.contactId = event.detail.recordId; }
    handleDateChange(event)    { this.callDate  = event.detail.value;    }
    handleOutcomeChange(event) { this.outcome   = event.detail.value;    }
    ...
    runClientValidation() {
        ...
        if (!allValid) {
            this.toast('Check the form', 'Please correct the highlighted fields.', 'warning');
        }
        return allValid;
    }
    ...
    handleEditStart(event) {
        ...
        this.recentNotes = this.recentNotes.map((n) =>
            n.Id === id
                ? { ...n, editing: true, draftNotes: n.Description, draftOutcome: n.Call_Outcome__c, draftDate: n.ActivityDate }
                : { ...n, editing: false }
        );
    }
    ...
    async handleEditSave(event) {
        ...
        if (!row.draftNotes || !row.draftNotes.trim()) {
            this.toast('Notes required', 'Please enter notes before saving.', 'warning');
            return;
        }
        try {
